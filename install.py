...
def install_packages():
    ...
    if os.path.exists("requirements.txt"):
        ...
        subprocess.check_call([pip_path, "install", "requests", "fastapi", "uvicorn"])
...
def create_env_file():
    ...
    if not os.path.exists(".env"):
        ...
        print("1. Todoist API Token (https://todoist.com/app/settings/integrations)")
        print("2. Telegram Bot Token (получить у @BotFather) - опционально")
        
        todoist_token = input("\nВведите Todoist API Token: ").strip()
        telegram_token = input("Введите Telegram Bot Token (или Enter для пропуска): ").strip()
        
        with open(".env", "w", encoding="utf-8") as f:
            f.write("# Todoist API Token (получен на https://todoist.com/app/settings/integrations)\n")
...
def check_todoist_setup():
    ...
    print("   💕 Отношения")
    print("   📚 Развитие")
    print("3. Получите API token: https://todoist.com/app/settings/integrations")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "qwen2.5:7b" in result.stdout:
                ...
                install_model = input("Установить модель qwen2.5:7b? (y/n): ").lower().strip()
                if install_model == 'y':
